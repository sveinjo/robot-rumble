...
func _load_textures():
	for i in range(1, 6):
		var hero_data: Dictionary = GameState.arrayHeroes[i]
		if hero_data and hero_data.has("texture_path"):
			hero_textures[i] = load(hero_data["texture_path"])

	for i in range(1, 8):
		var enemy_data: Dictionary = GameState.arrayEnemies[i]
...
func _setup_ambient_fx():
	...
	# if GameState.common_menu != null:
	#	GameState.common_menu.panel_open = true

func _generate_missions_if_needed():
	for i in range(1, 10):
		if GameState.arrayMissions[i] == null:
			_create_mission(i)
		else:
			var mission: Dictionary = GameState.arrayMissions[i]
			if mission.get("isNew", false) and not mission.has("fade_flag"):
				mission["fade_flag"] = true
				mission["image_alpha"] = 0.0
				GameState.arrayMissions[i] = mission
...
func _create_mission(index: int):
	...
	var mission_level := 1
	if index == 5:
		mission_level = max_level + 3 if GameState.base_overtaken else max_level
	else:
	...
	if not should_spawn:
		GameState.arrayMissions[index] = null
		return
...
func _update_mission_fades(delta: float):
	for i in range(1, 10):
		var raw_mission: Variant = GameState.arrayMissions[i]
		if raw_mission == null:
			continue
...
func _handle_left_click(mouse_pos: Vector2):
	for idx in range(1, 10):
		...
		if idx == 5 and not GameState.base_overtaken:
			if ResourceLoader.exists("res://features/home_base/scenes/home_base.tscn"):
				get_tree().change_scene_to_file("res://features/home_base/scenes/home_base.tscn")
			else:
...
func _draw_roster():
	...
	for i in range(1, 6):
		...
		var ability_name: String = str(GameState.arrayAbilities[ability_index]) if ability_index < GameState.arrayAbilities.size() else "Ability"
		var level := int(hero_data.get("intLevel", 1))
