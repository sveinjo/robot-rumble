...
func _install_common_menu():
	if common_menu != null:
		return
	if not ResourceLoader.exists("res://core/ui/common_menu/common_menu.gd"):
		return
	var menu_script: Script = load("res://core/ui/common_menu/common_menu.gd")
	if menu_script == null:
		return
...
func get_saved_scene_path(path: String = "user://savegame.json") -> String:
	if not FileAccess.file_exists(path):
		return ""
	var f := FileAccess.open(path, FileAccess.READ)
	if f == null:
